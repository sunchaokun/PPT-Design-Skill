"""Find and extract 3D components specifically, with full scheme resolution."""
                ...
                for lr in lr_root.findall(f"{{{_rel}}}Relationship"):
                    ...
                    if "theme" in lr_target.lower():
                        ...
                        if abs_theme in z.namelist():
                            return _parse_theme(z.read(abs_theme))
            ...
            with zipfile.ZipFile(fpath) as z:
                for sf in [n for n in z.namelist() if n.startswith("ppt/slides/slide") and n.endswith(".xml") and "rels" not in n]:
                    ...
                    for grp in sp_tree.iter(f"{{{_P}}}grpSp"):
                        ...
                        if has_3d and seen_3d >= 6:
                            continue
                        if not has_3d and seen_2d >= 6:
                            continue
                        ...
                        print(f"  Slide {slide_count+1} [{tag}]: {fname[:25]}, texts={t_count}, scheme_left={scheme_remaining}, theme={len(theme_colors)}, sample={texts[:3]}")
                        slide_count += 1
                        if has_3d:
                            seen_3d += 1
                        else:
...
out = os.path.join(OUT_DIR, "resolved_3d_vs_2d.pptx")
prs.save(out)
print(f"\nSaved: {out} ({os.path.getsize(out)/1024:.1f} KB)")
print(f"3D: {seen_3d}, 2D: {seen_2d}")
