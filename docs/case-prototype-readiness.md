...
本报告只记录当前仓库中可观察到的文件状态，不把 PNG 或 Markdown 说明自动视为可编辑 Prototype。合格 Prototype 必须能够从同一案例取得可重开的 PPTX、预览、Recipe、object map、素材许可和脱敏状态。这里明确区分：`build.py` 是可复现源代码，`output/*.pptx` 是已生成的候选源文件。项目负责人确认案例未经过手工修订，源码为唯一事实源；六个案例均已通过两次源码重建的 canonical content hash 比对。PPTX 是 ZIP 容器，raw SHA-256 仍可能因容器元数据变化而改变，因此 provenance 审计同时输出 canonical content hash。
